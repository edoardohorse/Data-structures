...
namespace PrintList{
	...
	template<typename TypeElem>
	void printStackPointer(NStackPointer::StackPointer<TypeElem>* list, int widthBuffer = 5){
		auto tmp = new NStackPointer::StackPointer<TypeElem>;
		while(!list->isEmpty()){
			tmp->push(list->top());
			list->pop();
		}
	...
	template<typename TypeElem>
	void printQueuePointer(NQueuePointer::QueuePointer<TypeElem>* list, int widthBuffer = 5){
		auto tmp = new NQueuePointer::QueuePointer<TypeElem>;
		while(!list->isEmpty()){
			tmp->push(list->top());
			list->pop();
		}
